...
const UsersTable = () => {
  ...
  const selectAll = (e) => e.target.checked ? setSelectedUsers(filteredUsers.map(u => u.id)) : setSelectedUsers([]);

  // Toggle status for single or multiple users
  const toggleStatus = (id) => {
    setUsers(users.map(u => u.id === id ? { ...u, status: u.status === "Active" ? "Blocked" : "Active" } : u));
  };

  const bulkToggleStatus = () => {
    setUsers(users.map(u => selectedUsers.includes(u.id) ? { ...u, status: u.status === "Active" ? "Blocked" : "Active" } : u));
    setSelectedUsers([]);
  };
  ...
  const deleteUser = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) setUsers(users.filter(u => u.id !== id));
    });
  };
      ...
      <div className="table-responsive">
        <table className="table table-dark table-hover table-striped align-middle">
          <thead>
            <tr>
              <th>
                <input type="checkbox" onChange={selectAll} checked={selectedUsers.length === filteredUsers.length && filteredUsers.length > 0} />
              </th>
          ...
          <tbody>
            {filteredUsers.map(user => (
              <tr key={user.id}>
                <td><input type="checkbox" checked={selectedUsers.includes(user.id)} onChange={() => toggleSelect(user.id)} /></td>
                <td>{user.id}</td>
                <td>{user.name}</td>
                <td>{user.email}</td>
                <td>{user.role}</td>
                ...
                <td>
                  <button className={`btn btn-sm ${user.status === "Active" ? "btn-warning" : "btn-success"} me-2`} onClick={() => toggleStatus(user.id)}>
